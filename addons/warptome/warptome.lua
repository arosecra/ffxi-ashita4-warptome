...
local function augural_conveyor(zone, test_mode)
	return lookup_by_location(zone, 'Augural Conveyor', skirmish_aliases, 'wp', test_mode)
end

local function crystal_warp(zone, test_mode)
    ...
    elseif (e.command == '/warptomeparty') then
		...
		if result ~= nil then
            ...
            for i=0,5 do
                ...
                warpto(result, name, false)
                coroutine.sleepf(30);
            end
		end
        e.blocked = true;
    elseif (e.command == '/warptomemuster') then
        local result = find_warp(false)
		if result ~= nil then
            --issue the muster command, but also set a flag to say that when it
            -- comes back we issue a warp command
		end
        e.blocked = true;
    end
